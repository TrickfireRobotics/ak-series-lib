...
<Callout type="info" title="Note">
    The destructor's bookkeeping — refusing to tear down while `Motor` objects are still alive —
    is being reworked, so don't rely on it catching a dangling `Motor` today.
</Callout>
...
<Callout type="warning" title="Caution">
    `sendDutyCycle` clamps against `limits.torque` (an Amps limit), not a dedicated duty-cycle
    bound — there isn't one in `MotorRunLimits` yet. Treat this as a placeholder.
</Callout>
